...
class Document_Loader {
	...
	public function load( $source, $mime = '' ) {
		...
		try {
			...
			return array( 'text' => '', 'title' => '', 'error' => $e->getMessage() );
		}
	}
	...
	protected function load_pdf( $source ) {
		if ( ! class_exists( '\\Smalot\\PdfParser\\Parser' ) ) {
			return array( 'text' => '', 'title' => '', 'error' => 'PDF parser library not installed.' );
		}

		$data = $this->fetch_bytes( $source );
		if ( is_wp_error( $data ) || ! $data ) {
			return array( 'text' => '', 'title' => '', 'error' => is_wp_error( $data ) ? $data->get_error_message() : 'Could not read PDF.' );
		}
	...
	protected function load_docx( $source ) {
		...
		if ( is_wp_error( $tmp ) ) {
			return array( 'text' => '', 'title' => '', 'error' => $tmp->get_error_message() );
		}

		if ( ! class_exists( 'ZipArchive' ) ) {
			return array( 'text' => '', 'title' => '', 'error' => 'ZipArchive not available.' );
		}

		$zip = new \ZipArchive();
		if ( true !== $zip->open( $tmp ) ) {
			$this->cleanup_temp( $tmp );
			return array( 'text' => '', 'title' => '', 'error' => 'Could not open DOCX archive.' );
		}
		...
		if ( false === $xml ) {
			return array( 'text' => '', 'title' => '', 'error' => 'DOCX missing word/document.xml.' );
		}
	...
	public function load_html( $source ) {
		...
		if ( is_wp_error( $raw ) ) {
			return array( 'text' => '', 'title' => '', 'error' => $raw->get_error_message() );
		}
	...
	protected function local_temp( $source ) {
		...
		$response = wp_remote_get( $source, array( 'timeout' => 60, 'redirection' => 5 ) );
		if ( is_wp_error( $response ) ) {
			return $response;
		}
		$code = (int) wp_remote_retrieve_response_code( $response );

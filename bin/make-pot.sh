#!/usr/bin/env bash
#
# Generate the languages/itih-ai-chatbot.pot translation template.
#
...
#
# Translators consume this .pot to produce itih-ai-chatbot-<locale>.po/.mo files
...
OUT="${ROOT_DIR}/languages/itih-ai-chatbot.pot"
...
if command -v wp >/dev/null 2>&1; then
    ...
    wp i18n make-pot . "${OUT}" \
        --domain="itih-ai-chatbot" \
        --package-name="ItihRag AI Chatbot" \
        --exclude="vendor,dist,.git,.github" \
        --headers='{"Report-Msgid-Bugs-To":"https://github.com/mahavirvataliya/itih-ai-chatbot/issues","POT-Creation-Date":""}' \
        --allow-root
    ...
    xgettext \
        ...
        --package-name="ItihRag AI Chatbot" \
        --msgid-bugs-address="https://github.com/mahavirvataliya/itih-ai-chatbot/issues" \
        --output="${OUT}" \
